...
export default function SignUpPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!email || !password || !confirmPassword) {
            toast.error("We need all your info to get you started!");
            return;
        }

        if (password !== confirmPassword) {
            toast.error("Hmm, those passwords are not matching up. Try again!");
            return;
        }

        if (password.length < 6) {
            toast.error("Make it a bit longer - 6 or more characters keeps things secure!");
            return;
        }
        ...
        if (error) {
            ...
            toast.success("You are in! Check your email to verify and let us go!");
            router.push("/login");
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                Sign Up <MoveRight className="w-4 h-4 ml-2" />
                            </>
                        )}
                    </Button>
                </form>
